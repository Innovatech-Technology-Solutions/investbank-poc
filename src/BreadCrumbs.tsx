...
const BreadCrumbs = ({ itemFeed, homePath, homeTitle='Home',showHome=true,onItemClick = () => {} }: BreadcrumbsProps) => {
  return (
          ...
          return (
            <li key={key}>
              {index !== itemFeed?.length - 1 ? (
                <Link onClick={() => onItemClick(item?.label)} to={item?.path} title={item?.label}>
                  {item?.label}
                </Link>
              ) : (
